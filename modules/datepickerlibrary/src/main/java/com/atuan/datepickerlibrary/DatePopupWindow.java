...
public class DatePopupWindow extends PopupWindow {
            ...
            public void onClick(View v) {
                if (mOnClickListener != null) {
                    ...
                    String endDate = mList.get(endGroupPosition)
                            ...
                            .getDate();
                    mOnClickListener.getDate(startDate, endDate, startGroupPosition, startChildPosition, endGroupPosition, endChildPosition,offsetDays);
                }
    ...
    private void getoffsetDate(String startDate, String endDate, boolean status) {
        ...
        offsetDays = Integer.parseInt(CalendarUtil.getTwoDay(endDate, startDate));
        if (offsetDays < 0) return;
        if (dayFalg) {
            tvTime.setText("共" + (offsetDays + 1) + "天");
        } else {
            tvTime.setText("共" + offsetDays + "晚");
        }
        //更改结束日期和完成按钮状态
    ...
    public interface DateOnClickListener {
        void getDate(String startDate, String endDate, int startGroupPosition, int startChildPosition, int endGroupPosition, int endChildPosition,int days);
    }
    ...
    public static class Builder {
        ...
        public Builder setMaxDay(int num) {
           this.maxDay=num;
            return this;
        }
                ...
                public void onItemClick(@NonNull BaseQuickAdapter<?, ?> adapter, @NonNull View view, int position) {
                    ...
                    if (status == 0 && endGroupPosition == -1 && endChildPosition == -1) {
                        ...
                        if (offset < 0) {
                            ...
                            tvHintText.setVisibility(View.VISIBLE);
                            return;
                        }else if(maxDay>-1&&offset>maxDay){
                            ToastUtil.INSTANCE.show("超出最大预定天数");
                            return;
                        }
                        //结束
        ...
        protected void convert(BaseViewHolder helper, DayInfo item) {
            ...
            if (status == 0) {
                if (isSelect) {
                    ...
                    ((TextView) helper.getView(R.id.tv_date)).setTextColor(activity.getResources().getColor(R.color.color_white));
                    (helper.getView(R.id.ll_bg)).setBackgroundColor(activity.getResources().getColor(R.color.color_theme60));
                } else {
                    //没选中状态
                    helper.getView(R.id.tv_date).setVisibility(View.VISIBLE);
                    helper.getView(R.id.tv_status).setVisibility(View.GONE);
                    helper.getView(R.id.tv_dateDel).setVisibility(View.GONE);
                    ((TextView) helper.getView(R.id.tv_date)).setTextColor(activity.getResources().getColor(R.color.color_black));
                    (helper.getView(R.id.ll_bg)).setBackgroundColor(activity.getResources().getColor(R.color.color_white));
                }
            } else if (status == 1) {
                //开始
                helper.getView(R.id.tv_date).setVisibility(View.VISIBLE);
                ...
                helper.getView(R.id.tv_dateDel).setVisibility(View.GONE);
                ((TextView) helper.getView(R.id.tv_status)).setTextColor(activity.getResources().getColor(R.color.color_white));
                ((TextView) helper.getView(R.id.tv_date)).setTextColor(activity.getResources().getColor(R.color.color_white));
                (helper.getView(R.id.ll_bg)).setBackgroundColor(activity.getResources().getColor(R.color.color_theme));
            } else if (status == 2) {
                //结束
                helper.getView(R.id.tv_date).setVisibility(View.VISIBLE);
            ...
            if (!isSelect && status == 0) {
                if (!isEnable) {
                    ...
                    textView.setTextColor(activity.getResources().getColor(R.color.color_gray_6B));
